...
* [Public] properties: w, h, speed, direction, lasttick, interval
* [Public] functions:
* - Initial()			| Pass 'Game' object to each particle object, used in 'func.h' (Using: Game)
* - UpdatePosition()	| Update particle properties(w, h) (Using: w, h, speed, direction, lasttick, interval)
*/
...
public:
	...
	double speed = ((0.1 * rand()) / RAND_MAX);
	int lasttick = 0;
	int interval = 0;
...
public:
	...
	void UpdatePosition(int _Tick, int _PreventPixelLossMode);
};
