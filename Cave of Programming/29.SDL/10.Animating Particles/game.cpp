...
void game::Draw(int _Height, int _Width, Uint8 _Red, Uint8 _Green, Uint8 _Blue) {
	...
	color <<= 8; color += _Blue;
	color <<= 8; color += Alpha;
	if (((_Height * SCREEN_WIDTH) + _Width) > SCREEN_WIDTH * SCREEN_HEIGHT || ((_Height * SCREEN_WIDTH) + _Width) < 0) {}	//* Ignore if formula output invalid buffer element  (Prevent access violation)
